...
Deterministic unit/contract tests cover empty and whitespace-only text, exact and one-over byte/line/line-byte bounds, malformed UTF-8/BOM variants, CRLF/CR/LF/final-empty-line rules, controls, noncharacters, bidi classes, unsupported scripts/emoji, combining runs 32/33 and valid/invalid Devanagari/Telugu joiner clusters. Literal markup/CSS/URL/SVG/template strings must remain escaped text and produce no resource escape.

Font tests require exact archive/file hashes, static Regular names/version/weight/tables/cmap, exact inventory/OFL notices and English/Hindi/Telugu/mixed shaping without system fallback. Renderer tests cover exact method/URL/context/source routing, no pass-through, cumulative byte/response caps before allocation, `IStream::Stat`, retained response lifetime, readiness/event order, all disabled settings/denials and stale/duplicate/cancelled/replacement authority. Service tests cover four page settings, boxes/rotation, qpdf arguments/failures, active content, encryption, forms/names/annotations/embedded files, embedded fonts, source/publication races, empty SQLite source paths, every unfinished TXT recovery state, UDF lock/retry and marker cleanup.
