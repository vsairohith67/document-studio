...
## G04E1 TXT-to-PDF matrix

Deterministic unit/contract tests cover empty and whitespace-only text, exact and one-over byte/line/line-byte bounds, malformed UTF-8/BOM variants, CRLF/CR/LF/final-empty-line rules, controls, noncharacters, bidi classes, rejected U+2028/U+2029 mandatory-break bypasses, unsupported scripts/emoji, combining runs 32/33 and valid/invalid Devanagari/Telugu joiner clusters. Literal markup/CSS/URL/SVG/template strings must remain escaped text and produce no resource escape.

Font tests require exact archive/file hashes, static Regular names/version/weight/tables/cmap, exact inventory/OFL notices and English/Hindi/Telugu/mixed shaping without system fallback. Renderer tests cover exact method/URL/context/source routing, no pass-through, cumulative byte/response caps before allocation, `IStream::Stat`, retained response lifetime, readiness/event order, exact wide path transfer, UDF identity replacement rejection, all disabled settings/denials and stale/duplicate/cancelled/replacement authority. Service tests cover four page settings, boxes/rotation, qpdf arguments/failures, active content, encryption, forms/names/annotations/embedded files, embedded fonts, source/publication races, post-move metadata failure adoption/mismatch preservation, empty SQLite source paths, every unfinished TXT recovery state, UDF lock/retry and marker cleanup.

The Windows acceptance harness first runs a real hidden-renderer fault matrix across STA initialization, environment/controller creation, boundary registration, navigation/readiness and print start/completion, including cancellation at every native checkpoint plus representative failure, timeout and crash outcomes. It then runs the accepted WebView2/qpdf path for all four page settings with synthetic mixed English/Hindi/Telugu and literal markup text. It must prove exact resource delivery, no denied request, page/font/security inventory, no-overwrite final hash equality and exact private cleanup. One already verified A4 result is copied only into marker-owned test evidence, reopened through the accepted PDF.js range transport, rendered to a nonblank A4-ratio canvas and checked for representative informational text extraction. Existing G03/G04, qpdf sandbox, single-instance, raw IPC, SEC1C hostile-environment and no-bundle build tests remain regressions.
