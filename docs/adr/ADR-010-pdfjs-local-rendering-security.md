...
No CDN or runtime fallback is allowed. Main API and worker versions must match. PDF.js receives bytes only through the opaque-session range transport. Explicit document settings include `isEvalSupported: false`, `enableXfa: false`, `stopAtErrors: true`, bounded range loading, local CMaps/fonts/ICC/WASM, and bounded image/canvas work. Document Studio does not create a scripting manager, annotation layer, attachment UI, form UI or external-link UI. Password callbacks live only in component memory; passwords are not persisted, logged, placed in diagnostics or sent to qpdf.
