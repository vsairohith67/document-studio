...
- Before Tauri context, plugin, window or WebView2 construction, Rust enumerates the process environment, removes every case-insensitive `WEBVIEW2_*` key and every casing of `COREWEBVIEW2_MAX_INSTANCES`, then sets exactly `COREWEBVIEW2_MAX_INSTANCES=20`. Production accepts no inherited browser-argument, runtime, channel, debugger, CDP, background or user-data override and retains Tauri/Wry's application-owned persistent profile. The feature-gated test runtime uses only explicit `DOCUMENT_STUDIO_TEST_*` controls plus direct builder UDF/CDP injection.
...
- CSP allows only self, required data/blob images/fonts, inline styles, same-origin workers, WebAssembly compilation through `wasm-unsafe-eval`, and Tauri's authenticated built-in `ipc:`/`http://ipc.localhost` command origins. The IPC origins are required for bounded raw RGBA request bodies and do not add a remote-network or app-defined custom-protocol path. CSP still denies `unsafe-eval`, external connects, objects, bases, frames and forms. Rust blocks outside-origin navigation/new windows.
...
## G04F1 batch-preview threats

- **TOCTOU source substitution:** preview retains restrictive source handles while hashing identity, size, modification time and content. Creation reopens and recomputes every ordered fingerprint; any proof change returns `BATCH_PLAN_STALE` before writes.
- **Destination/collision races:** Rust revalidates the canonical destination identity and recalculates the full collision plan immediately before the transaction. Windows `CompareStringOrdinal(..., TRUE)` supplies locale-independent ordinal-ignore-case comparison; component validation uses UTF-16 units and reserved-device rules before suffix selection.
- **Forged/stale plans:** the client receives no canonical payload to edit. Exact preview SHA-256, settings hash, operation/version, optimistic version and one-live-plan unique index fail closed. The transaction is `IMMEDIATE` and any insert fault rolls back every batch/job/input/output/spec/link row.
- **Path or content disclosure:** private canonical fingerprints and destination identity remain in Rust. The preview response exposes only safe display/output names, sizes, collision indexes, estimates and proof fields. No preview JSON, path, source hash, metadata, text or engine output is logged or synchronized.
- **Accidental execution:** the batch module cannot call `jobs_create`, worker registration, adapters, workspace creation or publication. Created children stay queued with planned output metadata and are excluded from startup worker recovery. No scheduler, retry or resume surface exists.
- **Dependency or arithmetic ambiguity:** preview verifies the accepted manifest-verified qpdf bundle and any existing private runtime cache read-only; it never materializes or repairs the cache. It proves destination add-file access with a directory handle and no probe file, then uses checked estimate arithmetic. Missing qpdf, denied destination access, overflow and insufficient exact-threshold space fail before durable creation.

## G04E1 TXT renderer threats

- **Markup/CSS injection:** Rust creates the entire fixed document and escapes `&`, `<` and `>` into one `<pre>`. User values never enter attributes, style, URLs, metadata, comments or raw fragments; script is disabled and the fixed CSP denies every class except same-origin style/fonts.
- **Network or profile escape:** a global all-context/all-source WebView2 filter is installed before navigation. Exact expected GET/context/source tuples receive retained memory-stream responses; wrong methods receive 405 and every other request receives 403 and terminates the generation. There is no pass-through, listener, loopback, file/data/custom protocol, proxy fallback, primary application UDF or external navigation.
- **Font confusion/fallback:** exact static Regular font bytes, names, version, OS/2 weight, required tables and cmap membership are preflighted. CSS has no generic/system fallback and disables synthesis; PDF inventory permits only the three approved PostScript names with embedded `FontFile2`.
- **Unicode abuse:** strict UTF-8, BOM/control/bidi/noncharacter admission, code-point range plus exact cmap membership and bounded combining/joiner shaping reject unsupported or pathological text before renderer creation. U+2028/U+2029 mandatory breaks are explicitly unsupported so Chromium layout cannot bypass the LF-owned logical-line and line-byte limits.
- **COM/callback ownership:** generated projections only, bounded `SHCreateMemStream`, `IStream::Stat` and retained buffers prevent short-stream/use-after-free paths. Job/generation/token/lifecycle/cancellation authority rejects stale, duplicate, late and replacement callbacks; a late-created stale controller is closed on its owning STA.
- **Resource exhaustion and recovery lock races:** every response is admitted against checked cumulative byte and retained-object counts before allocation. One operation deadline bounds all renderer, qpdf, verification, publication and cleanup work. The service retains no-delete directory guards from the application-owned workspace parent through the generation UDF, binds the UDF's Windows file identity into the render request and rechecks non-reparse identity immediately before and after environment creation. Startup proves exact markers and exclusive delete access for every UDF descendant before recursive cleanup; lock ambiguity preserves the full marker tree and remains retryable/interrupted.
- **Unsafe PDF or privacy residue:** private raw output is qpdf-normalized and inspected for encryption, boxes/rotation, active content/actions/names/forms/annotations/embedded/external data, metadata, source filename/path, username, UDF/workspace canaries and exact embedded fonts. Publication is no-overwrite and final bytes must match staging. A move-committed but not yet database-finalized TXT output retains its publishing path/hash/partial evidence; recovery adopts only an exact regular non-reparse hash match and otherwise preserves the user file plus retryable intent. Recovery accepts only exact markers and never deletes a published user file.
