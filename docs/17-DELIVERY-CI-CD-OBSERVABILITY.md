# Delivery, CI/CD and Observability

## Branch and release flow

- Protected `main`.
- Short-lived feature branches.
- Pull request requires specification validation, formatting, typecheck, tests and security/dependency review for new engines.
- Signed tags and release artifacts.
- Reproducible sidecar acquisition with checksums.

CI pins checkout, Python, Node, Rust toolchain and Cosign installer actions to immutable commit SHAs. Python installs with required hashes, npm uses `npm ci`, Cargo uses `--locked`, clippy denies warnings, and Windows compiles Tauri with `--no-bundle`. CI builds the isolated `test-runtime` feature and proves the single-instance boundary. For G02 it also acquires the signed qpdf archive into runner temporary storage, verifies provenance and every reviewed hash, compares all files byte-for-byte with the committed resource, then runs the real AppContainer/Job Object boundary test. The production Tauri build excludes the test probe. A dependency update must change its manifest and repeat licence/provenance review.

## Desktop packaging

- Windows MSI/NSIS first.
- macOS universal/architecture-specific package later.
- Optional engine bundles separated when licensing or size requires it.
- Application update channel is signed and can be disabled.

## Local observability

- Structured logs with correlation/job ID.
- Stage timings and engine version.
- No document text, password, key or prompt body by default.
- User-controlled diagnostics export with redaction.
- Local benchmark history to detect regressions.

Progress events and SQLite history use allow-listed fields rather than raw logs. Errors expose a typed safe code, title and detail. Dependency diagnostics never install or download an engine at runtime. The implementation log records commands, exits and limitations without copying document contents or secrets.

## G03 clean CI evidence

The Windows desktop job keeps immutable action SHAs and exact Node/Rust tools. After `npm ci --ignore-scripts`, CI runs the dependency-free PDF.js manifest/atomic-stage regressions, stages only exact allow-listed local assets, and independently checks membership/hash, dependency/worker parity, CSP, unchanged capabilities, raw range IPC and production/test separation. Optional package metadata may declare lifecycle scripts, but `--ignore-scripts` prevents execution. CI runs TypeScript, all Vitest/shared tests, production build and Playwright Chromium tests with `PLAYWRIGHT_BROWSERS_PATH` under the workspace cache. No browser is installed system-wide.

Cargo formatting, clippy and workspace tests include migration 4, opaque sessions, five qpdf operations, split/recovery, sandbox and leakage coverage. Windows then runs the real WebView2 raw-IPC smoke under `test-runtime`, the existing single-instance proof and Tauri `--no-bundle`. The test-runtime smokes inject hostile inherited WebView2 runtime, profile, browser-argument, debugger and future-variable controls while proving that only direct `DOCUMENT_STUDIO_TEST_*` UDF/CDP construction remains effective. The release smoke performs two ordinary launches, verifies one unchanged application-owned persistent profile and inspects only owned child processes for inherited CDP/debug controls. Production builds do not expose the test fixture command or any test-only environment control.

CI never downloads PDF.js or qpdf at runtime. npm acquisition occurs only through the exact lock during dependency installation; qpdf still follows its separately verified signed acquisition/reproduction step. Deterministic PDF fixtures are generated by tests and contain no third-party/user documents.

## G04E1 exact-head gates and diagnostics

Windows CI runs `verify:g04e1`, the complete locked Rust/frontend suite and the ignored-by-default real hidden WebView2/qpdf acceptance test explicitly. Browser preflight also regenerates one verified native A4 TXT PDF in owned evidence and requires the existing PDF.js worker/range/canvas/text-layer harness to render it and extract representative text. The verifier freezes migrations 1–8, direct dependency versions/checksums/features, the exact font/OFL inventory, no-network resource interception, COM stream/callback ownership, strict input limits, qpdf/security/publication/recovery paths, opaque IPC, accessible UI and the minimum Tauri capability set.

For pull requests, both validation jobs check out `github.event.pull_request.head.sha`, fail unless `HEAD` equals that SHA, fetch live `origin/main` without pruning, require it to equal the event base, require accepted main to be an ancestor and record exact head/tree/base identities. A separate lightweight job proves GitHub's merge-tree parents so integration remains covered without substituting the merge ref for exact-head execution. Only a run whose commit equals the PR head and whose base is current accepted main is admissible. Any repair requires a new ordinary commit and new exact-head run. PR cache save remains main-push-only. Sanitized runtime diagnostics may retain the WebView2 version, typed stage/error code, resource counts and output size/hash; they must not contain TXT bytes, filenames, source/destination/UDF/workspace paths, raw COM responses or PDF contents.

## Future cloud observability

- Metrics: queue wait, processing time by stage, failure code, worker saturation, upload/download duration and cleanup success.
- Distributed tracing without document content.
- Security audit logs and deletion proofs.
- SLOs defined only after real load tests.
