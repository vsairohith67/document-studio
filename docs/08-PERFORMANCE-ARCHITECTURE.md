...
# G04C2B bounded performance contract

Balanced compression is sequential: at most one decoded source image and one current encoded candidate are processed at a time, followed by one source/candidate page pair at a time. Hard limits include 256 image candidates, 128 affected pages, 8,192 pixels per axis, 16,777,216 pixels per page, 268,435,456 aggregate comparison pixels, four aggregate native range reads, 60 seconds per render side and 30 minutes for the visual stage. qpdf JSON, process output, object/depth, workspace space and candidate bytes are independently bounded. CPU metrics are authoritative; no GPU performance or quality claim is made.

The PR records the six-page corpus at 2,200,894 PDF bytes with 2,197,866 embedded JPEG bytes, zero selected replacements and six `candidate-not-smaller` skips. Focused single debug samples, excluding compilation, measured 7.48 seconds for corpus no-benefit, 4.60 seconds for the deterministic two-run qpdf candidate comparison, 20.45 seconds for the synthetic beneficial backend path without browser rendering, and 0.12 seconds for preflight cancellation and cleanup. These are not cold/warm percentile claims. Peak parent/qpdf/WebView memory, balanced cold/warm end-to-end timing and stage-separated decode/encode/qpdf/render/metric/publication timing remain unmeasured; no target is claimed for them and no safety or quality threshold was weakened.
