...
The G04B suite covers content-over-extension detection, JPEG/PNG/WebP, alpha soft masks, EXIF orientation, ICC warnings, selected-order MediaBoxes, qpdf strict/page-count verification, source immutability, malformed and oversized input, the exact 128-input acceptance/129-input rejection boundary, early and final-page cancellation, collision safety, injected publication failure, startup recovery, altered persisted-settings rejection, database constraints and accessible keyboard UI. A browser-backed test renders a generated first page through the accepted PDF.js viewer and compares its sampled pixel with the source image. Its native fixture producer is compiled in the `pretest:browser` lifecycle so a bounded cold Rust build does not consume Playwright's page-interaction timeout. Repository validation and the G04B boundary verifier pin the dependency, schema, scope, visual-evidence and capability invariants.
